...
def read_config(file_path: str, ass: dict[int, "AS"]) -> dict[int, "AS"]:
    if verbose:
        print("Step 1: Processing config file...")
    if verbose:
        print("=================================")
    ...
    for as_key, as_data in data["ASs"].items():
        ...
        for r_id, router in routers_dict.items():
            neighbours = []
            print(as_data)
            for interface_id in as_data["routers"][r_id]["neighbours"]:
                if interface_id in routers_dict:
                    neighbours.append(routers_dict[interface_id])
                    print(neighbours)
    ...
    for as_key, as_data in data["ASs"].items():
        ...
        for r_id, r_data in as_data["routers"].items():
            ...
            if "border_neighbours" in r_data:
                for bi in r_data["border_neighbours"]:
                    as_num, rid = bi.split(":")
                    as_num = int(as_num)
                    target_as = ass[as_num]
                    target_router = next(r for r in target_as.routers if r.ID == rid)
                    router.border_neighbours.append(target_router)
...
def generate_loopback_addresses(ass: dict[int, "AS"]) -> dict[int, "AS"]:
    if verbose:
        print(
            "Step 2: Generating and assigning loopback addresses to routers from AS loopback space..."
        )
    if verbose:
        print(
            "========================================================================================"
